...
-- Description:	Substitutes IntegrationSQL parameters into a canned script
-- =============================================
CREATE PROCEDURE [dbo].[SubstituteIntegrationSQLParameters] 
...
BEGIN
	...
	WHILE (@ctr <= @maxCtr)
	BEGIN
		SELECT @tempStr = FROMClause, @nameID = NameID, @SQLID = SQLID, @columnName = ColumnName
			FROM #TempSQL
			WHERE Sequence = @ctr
		INSERT #Parameters
			SELECT Name, ParameterData, ParameterDataType, ParameterIdentifier
				FROM IntegrationSQLReportIntegrationSQL 
				WHERE IntegrationSQLReportID = @nameID
				  AND IntegrationSQLID = @SQLID
				ORDER BY Name, ParameterIdentifier
	...
	BEGIN
		...
		INSERT #Parameters
			SELECT isql.ColumnName, isqlri.ParameterData, isqlri.ParameterDataType, isql.IntegrationSQLID
				FROM IntegrationSQLReportIntegrationSQL isqlri
					INNER JOIN IntegrationSQL isql ON isqlri.IntegrationSQLID = isql.IntegrationSQLID
				WHERE IntegrationSQLReportID = @nameID
				  AND isql.IntegrationSQLID = @SQLID
				ORDER BY isql.ColumnName, isql.IntegrationSQLID, isqlri.ParameterData desc
