...
-- =============================================
CREATE PROCEDURE [dbo].[RPT_CST_FC_BoxScore_ResidentActivity]
...
BEGIN
	...
	-- this update is custom for First Choice
	-- do logic to see if approved or not
	ALTER TABLE #CurrentOccupants ADD IsApproved bit null
	UPDATE #CurrentOccupants SET IsApproved = 0

	UPDATE #CurrentOccupants SET IsApproved = (SELECT CASE WHEN (pn.PersonNoteID IS NOT NULL) THEN 1 ELSE 0 END
												   FROM UnitLeaseGroup ulg
														INNER JOIN Lease l ON ulg.UnitLeaseGroupID = l.UnitLeaseGroupID
														INNER JOIN PersonLease pl ON l.LeaseID = pl.LeaseID
														INNER JOIN #PropertiesAndDates #pad ON #CurrentOccupants.PropertyID = #pad.PropertyID
														LEFT JOIN PersonNote pn ON pl.PersonID = pn.PersonID AND pn.PropertyID = #CurrentOccupants.PropertyID
																											 AND pn.InteractionType = 'Approved'
																									 		 AND pl.ApprovalStatus = 'Approved'
																											 AND pn.DateCreated <= #pad.EndDate
												   WHERE ulg.UnitLeaseGroupID = #CurrentOccupants.PendingUnitLeaseGroupID
												     AND pn.DateCreated > l.DateCreated
													 --AND #CurrentOccupants.OccupiedUnitLeaseGroupID IS NULL
													 AND #CurrentOccupants.PendingUnitLeaseGroupID IS NOT NULL
													 AND pl.PersonLeaseID = (SELECT TOP 1 pl1.PersonLeaseID	
																				...
																				WHERE pl1.LeaseID = l.LeaseID
																					AND pn1.PropertyID = #CurrentOccupants.PropertyID
													...
													AND pn.PersonNoteID = (SELECT TOP 1 pn2.PersonNoteID
																			   ...
																			   WHERE pn2.PersonID = pl.PersonID
																				...
																				AND pn2.PropertyID = #CurrentOccupants.PropertyID
...
--SELECT u.number, * FROM #CurrentOccupants #co
...
--order by u.paddednumber
	...
	-- need to update the pending move in date to NULL for ones that are not approved, otherwise keep it
	UPDATE #CurrentOccupants SET PendingMoveInDate = NULL WHERE IsApproved = 0 OR IsApproved IS NULL
	UPDATE #CurrentOccupants SET IsApproved = 0 WHERE IsApproved IS NULL
