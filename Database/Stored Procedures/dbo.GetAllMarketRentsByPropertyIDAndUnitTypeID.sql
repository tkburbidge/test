...
-- =============================================
CREATE PROCEDURE [dbo].[GetAllMarketRentsByPropertyIDAndUnitTypeID] 
...
BEGIN
	...
	INSERT #MyMarketRents
		SELECT p.PropertyID, ut.UnitTypeID, ut.Name, #myAPs.Sequence, null
			FROM Property p
	...
	UPDATE #mmr SET MarketRent = MarketRent.Amount
		FROM #MyMarketRents #mmr
			...
			CROSS APPLY GetLatestMarketRentByUnitTypeID(#mmr.UnitTypeID, #myAPs.EndDate) AS [MarketRent]
			
	SELECT	DISTINCT 
			PropertyID,
			UnitTypeID,
			Name,
			SUM(CASE WHEN (Sequence = 1) THEN ISNULL(MarketRent, 0) ELSE 0 END) AS 'Month1MarketRent',
			SUM(CASE WHEN (Sequence = 2) THEN ISNULL(MarketRent, 0) ELSE 0 END) AS 'Month2MarketRent',
			SUM(CASE WHEN (Sequence = 3) THEN ISNULL(MarketRent, 0) ELSE 0 END) AS 'Month3MarketRent',
			SUM(CASE WHEN (Sequence = 4) THEN ISNULL(MarketRent, 0) ELSE 0 END) AS 'Month4MarketRent',
			SUM(CASE WHEN (Sequence = 5) THEN ISNULL(MarketRent, 0) ELSE 0 END) AS 'Month5MarketRent',
			SUM(CASE WHEN (Sequence = 6) THEN ISNULL(MarketRent, 0) ELSE 0 END) AS 'Month6MarketRent',
			SUM(CASE WHEN (Sequence = 7) THEN ISNULL(MarketRent, 0) ELSE 0 END) AS 'Month7MarketRent',
			SUM(CASE WHEN (Sequence = 8) THEN ISNULL(MarketRent, 0) ELSE 0 END) AS 'Month8MarketRent',
			SUM(CASE WHEN (Sequence = 9) THEN ISNULL(MarketRent, 0) ELSE 0 END) AS 'Month9MarketRent',
			SUM(CASE WHEN (Sequence = 10) THEN ISNULL(MarketRent, 0) ELSE 0 END) AS 'Month10MarketRent',
			SUM(CASE WHEN (Sequence = 11) THEN ISNULL(MarketRent, 0) ELSE 0 END) AS 'Month11MarketRent',
			SUM(CASE WHEN (Sequence = 12) THEN ISNULL(MarketRent, 0) ELSE 0 END) AS 'Month12MarketRent'																																										
		FROM #MyMarketRents
		WHERE ISNULL(MarketRent, 0) <> 0
		GROUP BY PropertyID, UnitTypeID, Name
		ORDER BY PropertyID, Name
END
GO
