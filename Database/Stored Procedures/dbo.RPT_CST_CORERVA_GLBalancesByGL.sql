...
-- =============================================
CREATE PROCEDURE [dbo].[RPT_CST_CORERVA_GLBalancesByGL] 
...
BEGIN
	...
	UPDATE #AllInfo SET DebitAmount = (SELECT SUM(je.Amount)
												FROM JournalEntry je
													INNER JOIN [Transaction] t ON je.TransactionID = t.TransactionID
													--INNER JOIN #Properties p ON p.PropertyID = t.PropertyID
													INNER JOIN #PropertiesAndDates #pad ON t.PropertyID = #pad.PropertyID
	...
	UPDATE #AllInfo SET CreditAmount = (SELECT SUM(je.Amount)
												FROM JournalEntry je
													INNER JOIN [Transaction] t ON je.TransactionID = t.TransactionID
													--INNER JOIN #Properties p ON p.PropertyID = t.PropertyID
													INNER JOIN #PropertiesAndDates #pad ON t.PropertyID = #pad.PropertyID
	...
	-- Set bank accout name
	UPDATE #AllInfo SET BankAccountName = (SELECT TOP 1 ba.AccountName
												FROM BankAccount ba
													INNER JOIN BankAccountProperty bap ON ba.BankAccountID = bap.BankAccountID
												WHERE ba.GLAccountID = #AllInfo.GLAccountID
												  AND bap.PropertyID = #AllInfo.PropertyID)
	...
	BEGIN
		...
		--ELSE
		--BEGIN
			SELECT	PropertyID, GLAccountID, Number, Name, GLAccountType, BankAccountName,
					ISNULL(BeginningBalance, 0) AS 'BeginningBalance', ISNULL(DebitAmount, 0) AS 'DebitAmount',  ISNULL(-CreditAmount, 0) AS 'CreditAmount'
				FROM #AllInfo
