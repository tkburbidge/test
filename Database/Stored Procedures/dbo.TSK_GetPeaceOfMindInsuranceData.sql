...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[TSK_GetPeaceOfMindInsuranceData]
AS
BEGIN
	...
	SELECT 	
	   ...
	   (CASE WHEN (SELECT COUNT(*) FROM PersonLease WHERE LeaseID = l.LeaseID AND MoveOutDate IS NULL) = 0 
		...
		CASE WHEN ri.RentersInsuranceID IS NULL THEN NULL
			 WHEN ri.IntegrationPartnerItemID = 192 THEN 'Peace of Mind' 
			 WHEN ri.IntegrationPartnerItemID = 223 THEN 'Peace of Mind Owner' 
			 ELSE 'Private'
	...
	FROM Unit u
	INNER JOIN Settings s ON s.AccountID = u.AccountID
	...
	AND ipip.IntegrationPartnerItemID IN (129, 223)
