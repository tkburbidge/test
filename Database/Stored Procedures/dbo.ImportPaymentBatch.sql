...
-- =============================================
CREATE PROCEDURE [dbo].[ImportPaymentBatch] 
...
BEGIN
	...
	INSERT #NewPayments 
		SELECT p.*, null FROM @payments p
			LEFT JOIN ProcessorPayment pp ON p.PartnerTransactionID = pp.ProcessorTransactionID AND pp.AccountID = @accountID --AND pp.ObjectID = p.ObjectID	2016-12-13: ObjectID from PayLease can change between the time of transaction and settlement if we call SetResidents with a different ID		
		...
		BEGIN
			...
			INSERT [Transaction] (TransactionID, AccountID, ObjectID, TransactionTypeID, LedgerItemTypeID, 
					...
					VALUES		(@newTransID, @accountID, @objectID, @transactionTypeID, COALESCE(@ledgerItemTypeID, @defaultLedgerItemTypeID),
									...
									@description, null, @date, @postingBatchID, GETDATE()) 
			...
			BEGIN
				INSERT ActivityLog (ActivityLogID, AccountID, ActivityLogType, ModifiedByPersonID, ObjectName, ObjectID, PropertyID,
					...
					VALUES			(NEWID(), @accountID, 'ImportPaymentBatch', @personID, @objectName, @objectID, @propertyID,
