...
-- =============================================
CREATE PROCEDURE [dbo].[PerformanceUtils]	
AS
BEGIN
	...
	BEGIN
		...
		BEGIN 
		  ...
		  BEGIN 
			...
			RAISERROR(@Message, 0, 1) WITH NOWAIT 
			SET @cmd = 'ALTER INDEX ' + QUOTENAME(@IndexName) + ' ON [' + RTRIM(LTRIM(@SchemaName)) + '].[' + RTRIM(LTRIM(@Table)) + '] REORGANIZE'  
			EXEC (@cmd) 
			PRINT @cmd  
		  ...
		  BEGIN 
			...
			--SET @cmd = 'ALTER INDEX ' + @IndexName + ' ON [' + RTRIM(LTRIM(@SchemaName)) + '].[' + RTRIM(LTRIM(@Table)) + '] REBUILD WITH (FILLFACTOR = ' + CONVERT(VARCHAR(3),@fillfactor) + ', STATISTICS_NORECOMPUTE = OFF, ONLINE = ON)' 
			SET @cmd = 'ALTER INDEX ' + QUOTENAME(@IndexName) + ' ON [' + RTRIM(LTRIM(@SchemaName)) + '].[' + RTRIM(LTRIM(@Table)) + '] REBUILD WITH (FILLFACTOR = ' + CONVERT(VARCHAR(3),@fillfactor) + ', STATISTICS_NORECOMPUTE = OFF, ONLINE = OFF)' 
			EXEC (@cmd) 
			PRINT @cmd 
	...
	EXEC sp_recompile 'GetGLAccountRegister'
	EXEC sp_recompile 'GetOutstandingCharges'
	EXEC sp_recompile 'GetUnappliedPayments'
	EXEC sp_recompile 'RPT_TNS_AgedReceivables'
	EXEC sp_recompile 'GetBankTransactionLedger'
	EXEC sp_recompile 'SearchAll'
	EXEC sp_recompile 'RPT_INV_UnpaidInvoices'
	EXEC sp_recompile 'RPT_INV_GetAPAgingSummaries'
	EXEC sp_recompile 'GetDepositablePayments'
	EXEC sp_recompile 'RPT_TNS_Prepaids'
	EXEC sp_recompile 'RPT_TNS_Delinquency'
