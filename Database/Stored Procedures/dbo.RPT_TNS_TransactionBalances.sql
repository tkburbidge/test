...
-- =============================================
CREATE PROCEDURE [dbo].[RPT_TNS_TransactionBalances] 
...
BEGIN
	...
	INSERT #PropertiesAndDates 
		SELECT pIDs.Value, COALESCE(pap.StartDate, @startDate)
		...
		FROM (SELECT DISTINCT p.PaymentID, p.Amount
				...
				WHERE t.ObjectID = #TransactionBalance.ObjectID
				  ...
				  AND (((@accountingPeriodID IS NULL) AND (p.[Date] >= @startDate) AND (p.[Date] <= @endDate))
				    OR ((@accountingPeriodID IS NOT NULL) AND (p.[Date] >= pap.StartDate) AND (p.[Date] <= pap.EndDate)))) DistinctPayments)   
	...
	UPDATE #TB SET BeginningBalance = Bal.Balance
		FROM #TransactionBalance #TB
			--CROSS APPLY GetObjectBalance(null, DATEADD(DAY, -1, @startDate), #TB.ObjectID, 0, @propertyIDs) AS [Bal]
			INNER JOIN #PropertiesAndDates #pad ON #TB.PropertyID = #pad.PropertyID
			CROSS APPLY GetObjectBalance(null, DATEADD(DAY, -1, #pad.StartDate), #TB.ObjectID, 0, @propertyIDs) AS [Bal]
