...
-- =============================================
CREATE PROCEDURE [dbo].[ImportCreditBatch] 
...
--DECLARE @leaseTTGLAccountID			uniqueidentifier
DECLARE @transactionTypeID			uniqueidentifier
DECLARE @prepaymentTTypeID			uniqueidentifier
...
BEGIN
	...
	-- Insert the payments into a temp table with a sequence number		  
	INSERT #NewPayments SELECT * FROM @payments
	SET @maxSequenceNumber = (SELECT MAX(SequenceNum) FROM #NewPayments)
	SET @ctr = 1
	
	WHILE (@ctr <= @maxSequenceNumber)
	BEGIN
		--SET @TTGLAccountID = @leaseTTGLAccountID
		SET @transactionTypeID = null
		SET @objectName = NULL
		
		-- Pull out the data needed for the payment
		SELECT @objectID = ObjectID, @objectName = ReceivedFrom, @type = [Type], @referenceNumber = ReferenceNumber,
				@amount = Amount, @date = [Date], @description = [Description], @ledgerItemTypeID = LedgerItemTypeID,
				@payerPersonID = PayerPersonID
			FROM #NewPayments
			WHERE SequenceNum = @ctr
			
		-- If a payment has already been posted to this objectID
		-- just grab the TransactionTypeID from that payment			
		...
		BEGIN
			...
			SET @newPrepaymentTransID = NEWID()
			INSERT [Payment] (PaymentID, AccountID, [Type], ReferenceNumber, [Date], ReceivedFromPaidTo, ObjectID, ObjectType,
				...
				VALUES (@newPaymentID, @accountID, @type, @referenceNumber, @date, @objectName, @objectID, @objectType,
								@amount, @description, @note, CAST(0 AS BIT), CAST(0 AS BIT), @postingBatchID, GETDATE(), @payerPersonID)
			INSERT [Transaction] (TransactionID, AccountID, ObjectID, TransactionTypeID, LedgerItemTypeID, 
									PropertyID, PersonID, TaxRateGroupID, NotVisible, IsDeleted, Origin, Amount,
									[Description], Note, TransactionDate, PostingBatchID, [TimeStamp])
					VALUES		(@newTransID, @accountID, @objectID, @transactionTypeID, COALESCE(@ledgerItemTypeID, @defaultLedgerItemTypeID),
									@propertyID, @personID, @taxRateGroupID, 0, 0, @origin, @amount,
									@description, @note, @date, @postingBatchID, GETDATE()) 
			IF (@@ROWCOUNT < 0)
			BEGIN
				INSERT ActivityLog (ActivityLogID, AccountID, ActivityLogType, ModifiedByPersonID, ObjectName, ObjectID, PropertyID,
					...
					VALUES			(NEWID(), @accountID, 'ImportingCreditBatch', @personID, @objectName, @objectID, @propertyID,
		...
		BEGIN
			INSERT ActivityLog	(ActivityLogID, AccountID, ActivityLogType, ModifiedByPersonID, ObjectName, ObjectID, PropertyID,
				...
				VALUES			(NEWID(), @accountID, 'ImportingCreditBatch', @personID, @objectName, @objectID, @propertyID,
									...
									'Credit for ' + @objectName + ', Reference: ' + @referenceNumber + ', Amount $' + CAST(@amount as nvarchar) + ', Description: ' + @description + ', Note: ' + @note, @postingBatchID)
		END
		SET @ctr = @ctr + 1			
	END
