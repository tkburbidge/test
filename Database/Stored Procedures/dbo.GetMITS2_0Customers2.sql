...
--				interface
-- =============================================
CREATE PROCEDURE [dbo].[GetMITS2_0Customers2]
...
BEGIN
	...
	SELECT * FROM (
		SELECT DISTINCT				
			...
			(CASE WHEN l.LeaseStatus IN ('Pending', 'Pending Transfer') THEN
					(SELECT COUNT(*) 
					 ...
					 WHERE pl3.LeaseID = l.LeaseID 
						AND pl3.ResidencyStatus NOT IN ('Cancelled'))						
				  ELSE
		  			(SELECT COUNT(*) 
					 FROM PersonLease pl3
					 INNER JOIN PickListItem pli ON pli.Name = pl3.HouseholdStatus AND (pli.IsNotOccupant IS NULL OR pli.IsNotOccupant = 0)
			...
			(CASE WHEN l.LeaseStatus = 'Former' THEN (SELECT MAX(pl3.MoveOutDate)
														FROM PersonLease pl3
														WHERE pl3.LeaseID = l.LeaseID 
														AND pl3.ResidencyStatus NOT IN ('Cancelled'))
				  ELSE NULL
		...
		INNER JOIN Lease l ON l.UnitLeaseGroupID = ulg.UnitLeaseGroupID AND LeaseStatus IN ('Current', 'Former', 'Pending', 'Pending Transfer')
		INNER JOIN PersonLease pl ON pl.LeaseID = l.LeaseID AND pl.ResidencyStatus NOT IN ('Cancelled')
		-- Don't include non-occupant HouseholdStatuses
		INNER JOIN PickListItem pli ON pli.Name = pl.HouseholdStatus AND (pli.IsNotOccupant IS NULL OR pli.IsNotOccupant = 0)
		...
		WHERE 		
			...
			AND prop.PropertyID = @propertyID) AS Customers
	WHERE
